...
export const ProtocolList = () => {
  ...
  const handlePrintAll = () => {
    ...
    if(typeFilter !== 'ALL') params.append('protocolType', typeFilter);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-gray-800">Gesprächsprotokolle</h2>
        ...
        <div className="flex gap-2">
            ...
            <Link to="/protocols/new" className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition flex items-center shadow-sm">
                <Plus className="w-5 h-5 mr-2" /> Neues Gespräch
            </Link>
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <div className="relative min-w-[180px]">
            <select 
              ...
              className="w-full appearance-none pl-3 pr-8 py-2 border border-gray-300 rounded-md bg-white focus:ring-2 focus:ring-indigo-500 outline-none text-sm"
            >
              <option value="ALL">Alle Gesprächsarten</option>
              {Object.values(ConversationType).map(t => <option key={t} value={t}>{t}</option>)}
            </select>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200 text-gray-500 text-xs uppercase tracking-wider">
                <th className="px-6 py-4 font-semibold">Datum / Typ</th>
                <th className="px-6 py-4 font-semibold">Teilnehmer</th>
                <th className="px-6 py-4 font-semibold">Thema / Inhalt</th>
                <th className="px-6 py-4 font-semibold">Ergebnis</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                filteredConversations.map(conv => (
                  <tr key={conv.id} onClick={() => navigate(`/protocols/edit/${conv.id}`)} className="hover:bg-indigo-50 transition-colors cursor-pointer group">
                    <td className="px-6 py-4">
                      <div className="font-medium text-gray-800">{new Date(conv.date).toLocaleDateString()}</div>
                      <div className="text-xs text-indigo-600 font-medium">{conv.type}</div>
                      <div className="text-xs text-gray-400">{conv.time} Uhr</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-medium text-gray-900">{conv.studentName || 'Ohne Schülerbezug'}</div>
                      <div className="text-xs text-gray-500">{conv.className ? `Klasse ${conv.className}` : ''}</div>
                      {conv.participants && <div className="text-xs text-gray-600 mt-1 flex items-center"><Users className="w-3 h-3 mr-1"/> {conv.participants}</div>}
                    </td>
                    <td className="px-6 py-4 max-w-xs">
                      <div className="font-medium text-gray-800 text-sm truncate">{conv.subject}</div>
                      <div className="text-xs text-gray-500 truncate">{conv.content}</div>
                    </td>
                    <td className="px-6 py-4 max-w-xs">
                       <div className="text-sm text-gray-600 truncate">{conv.results || '-'}</div>
                       {conv.nextAppointment?.date && (
                         <div className="text-xs text-amber-600 mt-1 flex items-center">
                            <Calendar className="w-3 h-3 mr-1"/> Folgetermin: {new Date(conv.nextAppointment.date).toLocaleDateString()}
                         </div>
                       )}
                    </td>
